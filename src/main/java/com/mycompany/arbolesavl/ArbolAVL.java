...
public class ArbolAVL <T extends Integer> {
    ...
    public String toString() {
        System.out.println(
            "Arbol Inorden ( " + inOrden(this.raiz) + 
            ")\nArbol PreOrden ( " + preOrden(this.raiz) +
            ")\nArbol PostOrden ( " + postOrden(this.raiz) +
            ")\nLa altura es: " + altura() +
            "\nNumero de Nodos: " + numeroDeNodos() + 
            "\nLas hojas son: " + obtenerHojas()
        );
        return "";
    }

    public ListaSimple<NodoArbol<T>> obtenerHojas() {
        ListaSimple<NodoArbol<T>> hojas = new ListaSimple<>();
        obtenerHojas(this.raiz, hojas);
        return hojas;
    }

    private void obtenerHojas(NodoArbol<T> nodo, ListaSimple<NodoArbol<T>> hojas) {
        if (nodo != null) {
            if (nodo.izq == null && nodo.der == null) {
                // El nodo actual es una hoja
                hojas.add(nodo);
            }
            obtenerHojas(nodo.izq, hojas);
            obtenerHojas(nodo.der, hojas);
        }
    }
    

    public int numeroDeNodos() {
        return numeroDeNodos(this.raiz);
    }

    private int numeroDeNodos(NodoArbol<T> nodo) {
        if(nodo == null) {
            return 0;
        }
        return 1 + numeroDeNodos(nodo.der) + numeroDeNodos(nodo.izq);
    }
